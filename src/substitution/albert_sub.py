...
class AlbertSubstitutor(BaseSubstitutor):
    '''
    ...
    def __init__(self, model_type, model_dir = './model_cache/albert_model/albert-base-v2/masklm/',
                filter_words_file = './aux_files/vocab.txt', mask_orig = False):
        '''
        filter words file: some stop words from the vocabulary of BERT
        '''
        super().__init__()
        self.tokenizer = AlbertTokenizer.from_pretrained(model_type, cache_dir = model_dir)
        self.albert_model = AlbertForMaskedLM.from_pretrained(model_type,cache_dir = model_dir).to('cuda')
        ...
        self.stopwords.append('▁') ## token for white space in ALBERT
        self.antonym_filter = WordNetFilter()
        self.mask_orig = mask_orig

        # print(self.stopwords)

    def get_neighbor_list(self, word_list, site_mask = None, k = 50, threshold_pred_score = 0.1, pos_list = []):
    ...
    def parse_sentence(self, sub_words, position = 0, k = 50, threshold_pred_score = 0.1, site_mask: torch.LongTensor = None, word_pos = 'n'):
        '''
        ...
        if detokenized_orig_word in self.stopwords or original_word in self.stopwords:
            return [], [], 0
        if self.mask_orig:
        ...
        word_predictions = self.albert_model(input_ids_.to('cuda'))[0].squeeze()[position]  # vocab
    ...
    def get_substitues(self, substitutes, substitute_score = None, substitute_probability = None, threshold = 0):
        words = []
        probability_list = []
        detokenized_words = []
